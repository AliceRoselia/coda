...
mod cuckoo_tests {
    ...
    /// Slow oracle: `has_game_cycle(board, ply)` should match "there exists a
    /// legal move after which `is_repetition_draw(ply + 1)` is true".
    #[test]
        ...
        for (fen_idx, fen) in FENS.iter().enumerate() {
            for game in 0..GAMES {
                ...
                for step in 0..PLIES {
                    ...
                    let ply_cases = [0, (stack_len / 2).max(0), stack_len, stack_len + 8];
