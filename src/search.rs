...
fn quiescence_with_depth(
    ...
    // Apply the SAME halfmove guard as the direct cutoff path above: without
    // this, an inflated near-50mr TT lower bound replaces stand_pat and
    // triggers the `best_score >= beta` return below — bypassing the cutoff
    // gate that exists for exactly this reason.
