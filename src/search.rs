...
tunables!(
    ...
    (HIST_PRUNE_MULT, 12495, 500, 50000, 2475.0),
    (SEE_QUIET_MULT, 32, 5, 80, 3.75),
    (LMR_HIST_DIV, 7248, 2000, 100000, 4900.0),
    (LMR_C_QUIET, 138, 40, 300, 13.0),
    (LMR_C_CAP, 97, 80, 350, 12.5),
    (SE_DEPTH, 4, 4, 20, 2.0),
    ...
    (HIST_BONUS_MULT, 327, 50, 400, 17.5),
    (HIST_BONUS_MAX, 1694, 500, 3000, 125.0),
    ...
    (CAP_HIST_MULT, 276, 50, 400, 17.5),
    (CAP_HIST_BASE, 32, 0, 200, 10.0),
    (CAP_HIST_MAX, 1664, 500, 3000, 125.0),
...
pub struct SearchInfo {
    ...
    /// Per-ply move context for cont-hist parent-bucket selection:
    ///   bit 0 = in_check at parent (was side-to-move in check before move)
    ///   bit 1 = parent move was a capture
    /// 2-bit value (0-3) directly indexes `cont_hist[bucket][...]`.
    /// See `History::cont_hist` doc.
    move_context_stack: [u8; MAX_PLY + 1],
...
impl SearchInfo {
    pub fn new(tt_mb: usize) -> Self {
        SearchInfo {
            ...
            move_context_stack: [0; MAX_PLY + 1],
...
fn search_helper(board: &mut Board, info: &mut SearchInfo, _limits: &SearchLimits, thread_id: usize) -> Move {
    ...
    info.move_context_stack = [0; MAX_PLY + 1];
...
fn negamax(
                    ...
                    && halfmove_ok
                {
                    ...
                    if score_above_beta && stack_len >= 2 && ply_u >= 2 {
                            ...
                            && our_undo.mv != NO_MOVE
                        {
                            ...
                            // Bucket: parent-of-(ss-1) is the (ss-2) move; cont-hist
                            // for (ss-1) → (ss-1)'s move is bucketed by (ss-2)'s context.
                            let bucket = (info.move_context_stack[ply_u - 2] & 0x3) as usize;
                                ...
                                && opp_to < 64 && our_to < 64
                            {
                                ...
                                History::update_cont_history(
                                    &mut info.history.cont_hist[bucket][our_gp][our_to][opp_gp][opp_to],
                                    malus,
                                );
                            }
                        }
        ...
        && FEAT_NMP.load(Ordering::Relaxed)
    {
        ...
        if ply_u <= MAX_PLY {
            ...
            info.move_context_stack[ply_u] = 0; // null move: bucket 0 (no check, no capture)
        }
    ...
    let mut picker = if in_check {
        MovePicker::new_evasion(tt_move, safe_ply, checkers, pinned, &info.history, prev_move, pawn_hist_ref, enemy_attacks, &info.moved_piece_stack, &info.moved_to_stack, &info.move_context_stack)
    } else {
        MovePicker::new(board, tt_move, safe_ply, &info.history, prev_move, pawn_hist_ref, enemy_attacks, our_xray_blockers, &info.moved_piece_stack, &info.moved_to_stack, &info.move_context_stack)
    };
    ...
    loop {
        ...
        if moved_piece != NO_PIECE && ply_u <= MAX_PLY {
            ...
            // Cont-hist parent-bucket context: in_check (bit 0) | is_cap (bit 1).
            info.move_context_stack[ply_u] = (in_check as u8) | ((is_cap as u8) << 1);
        }
            ...
            if moved_piece != NO_PIECE {
                ...
                for &off in &offsets {
                    if ply_u >= off {
                        ...
                        let bucket = (info.move_context_stack[ply_u - off] & 0x3) as usize;
                        if p > 0 && p < 13 && pt < 64 {
                            hist_prune_score += info.history.cont_hist[bucket][p][pt][gp][to as usize] as i32;
                        }
                    }
                }
            ...
            if moved_piece != NO_PIECE {
                ...
                for (i, &off) in offsets.iter().enumerate() {
                    if ply_u >= off {
                        ...
                        let bucket = (info.move_context_stack[ply_u - off] & 0x3) as usize;
                        if p > 0 && p < 13 && pt < 64 {
                            let v = info.history.cont_hist[bucket][p][pt][gp][to as usize] as i32;
            ...
            if moved_piece != NO_PIECE && threshold > 0 {
                ...
                for (i, &off) in offsets.iter().enumerate() {
                    if ply_u >= off {
                        ...
                        let bucket = (info.move_context_stack[ply_u - off] & 0x3) as usize;
                        if p > 0 && p < 13 && pt < 64 {
                            conts[i] = info.history.cont_hist[bucket][p][pt][gp][to as usize] as i32;
                        }
                    }
                }
        ...
        if !in_check && !is_cap && !is_promo && !is_endgame_skip && FEAT_LMR.load(Ordering::Relaxed) {
            ...
            if reduction > 0 {
                ...
                if moved_piece != NO_PIECE {
                    let gp = go_piece(moved_piece);
                    if prev_piece_for_cont != 0 && ply_u >= 1 {
                        let bucket = (info.move_context_stack[ply_u - 1] & 0x3) as usize;
                        hist_score += info.history.cont_hist[bucket][prev_piece_for_cont][prev_to_for_cont as usize][gp][to as usize] as i32;
                    }
                    if prev2_piece_for_cont != 0 && ply_u >= 2 {
                        let bucket = (info.move_context_stack[ply_u - 2] & 0x3) as usize;
                        hist_score += info.history.cont_hist[bucket][prev2_piece_for_cont][prev2_to_for_cont as usize][gp][to as usize] as i32 / 2;
                    }
                            ...
                            for (i, &off) in ch_offsets.iter().enumerate() {
                                if ply_u >= off {
                                    ...
                                    if prior_piece > 0 && prior_piece < 12 && prior_to < 64 {
                                        ...
                                        History::update_cont_history(
                                            &mut info.history.cont_hist[bucket][prior_piece][prior_to][gp_mv][to as usize],
                                            ch_bonus,
                                        );
                                    ...
                                    for (i, &off) in ch_offsets.iter().enumerate() {
                                        if ply_u >= off {
                                            ...
                                            if prior_piece > 0 && prior_piece < 12 && prior_to < 64 {
                                                ...
                                                History::update_cont_history(
                                                    &mut info.history.cont_hist[bucket][prior_piece][prior_to][gp_q][qt as usize],
                                                    ch_pen,
                                                );
                                                info.stats.cont_hist_writes[i] += 1;
...
fn quiescence_with_depth(
    ...
    if qs_in_check {
        ...
        let mut evasion_picker = MovePicker::new_evasion(
            ...
            qs_enemy_attacks,
            &info.moved_piece_stack, &info.moved_to_stack, &info.move_context_stack,
        );
        let mut best_score = -INFINITY;
        let mut best_move = NO_MOVE;
        let mut move_count = 0i32;
