...
tunables!(
    ...
    (LMR_C_QUIET, 144, 80, 300, 13.0, true),   // widened lower bound: base frees C to rise toward peers' 200-314
    // Base offset for quiet LMR — lifts reductions at shallow (d=3-5, m=3-8).
    // Every peer above Coda uses base + ln(d)*ln(m)/C. Without base, SPSA
    // drove C to 1.44 (peers 2.0-3.1), creating a d=3-5 deadzone. (LMR audit L1)
    (LMR_BASE_QUIET, 50, 0, 150, 7.5, true),
...
pub fn init_lmr() {
    for depth in 1..64 {
        for moves in 1..64 {
            // Quiet table: base + ln(d)*ln(m)/C (LMR audit L1).
            if depth >= 3 && moves >= 3 {
