...
tunables!(
    ...
    (NMP_DEPTH_DIV_10X, 90, 10, 200, 15.0, true),
    (NMP_EVAL_DIV, 127, 50, 400, 17.5, true),
    (NMP_EVAL_MAX_10X, 35, 10, 60, 5.0, true),
    (NMP_VERIFY_DEPTH_10X, 176, 40, 200, 20.0, true),
    (RFP_DEPTH, 9, 2, 20, 2.0, true),
    ...
    (FUT_BASE, 4, 0, 200, 9.0, true),
    (FUT_PER_DEPTH, 42, 40, 250, 10.5, true),
    (HIST_PRUNE_DEPTH_10X, 11, 10, 80, 15.0, true),
    (HIST_PRUNE_MULT, 15157, 500, 50000, 2475.0, true),
    (SEE_QUIET_MULT, 15, 5, 80, 3.75, true),
    (LMR_HIST_DIV, 2821, 2000, 100000, 4900.0, true),
    ...
    (LMR_HIST_DIV_CAP, 5441, 1000, 20000, 1500.0, true),
    (LMR_C_QUIET, 166, 40, 300, 13.0, true),
    (LMR_C_CAP, 137, 80, 350, 12.5, true),
    ...
    (SE_DEPTH_10X, 67, 40, 200, 20.0, true),
    (ASP_DELTA, 16, 5, 30, 1.5, false),
    (ASP_SCORE_DIV, 16100, 8000, 50000, 2100.0, false),
    ...
    // Bisecting 9 → 5 first.
    (LMP_BASE, 14, 1, 15, 2.0, true),
    (LMP_DEPTH, 7, 4, 20, 2.0, true),
    (BAD_NOISY_MARGIN, 99, 30, 150, 6.0, true),
    (PROBCUT_MARGIN, 166, 80, 300, 11.0, true),
    (HINDSIGHT_THRESH, 149, 50, 400, 17.5, true),
    (UNSTABLE_THRESH, 234, 50, 500, 22.5, false),
    (SEE_MATERIAL_SCALE, 179, 30, 300, 13.5, false),
    (QS_DELTA_MARGIN, 375, 100, 500, 20.0, true),
    (QS_SEE_THRESHOLD, -24, -200, 0, 10.0, false),
    (QS_MAX_CAPTURES, 29, 2, 32, 2.0, false),
    (CORR_W_PAWN, 260, 100, 600, 25.0, true),
    // Floor lifted from 50 → 0 (audit 2026-05-20): pinned at 63, 4% from floor.
    (CORR_W_NP, 72, 0, 400, 17.5, true),
    ...
    (CORR_W_CONT, 16, 0, 400, 18.5, true),
    (FH_BLEND_DEPTH_10X, 35, 0, 80, 15.0, false),
    ...
    (TT_DAMP_TT_WEIGHT, 2, 1, 10, 0.5, false),
    ...
    (PROBCUT_TT_DEPTH_SLACK, 6, 0, 10, 0.5, false),
    ...
    (HIST_BONUS_MULT, 287, 50, 400, 17.5, true),
    (HIST_BONUS_MAX, 1173, 500, 3000, 125.0, true),
    ...
    (HIST_BONUS_OFFSET, 40, 0, 400, 25.0, false),
    (CAP_HIST_MULT, 399, 50, 400, 17.5, true),
    (CAP_HIST_BASE, 37, 0, 200, 10.0, false),
    (CAP_HIST_MAX, 1831, 500, 3000, 125.0, true),
    ...
    (NFH_CAP_10X, 59, 10, 60, 10.0, false),
    ...
    (NFH_DIV_10X, 26, 20, 120, 10.0, false),
    ...
    (DEXT_MARGIN_PV, 224, 50, 400, 15.0, true),
    (DEXT_MARGIN_QUIET, 12, 0, 100, 4.0, true),
    (DEXT_MARGIN_CORR, 30, 0, 64, 3.0, true),
    (DEXT_MARGIN_BASE, 65, -50, 150, 6.0, true),
    (DEXT_CAP, 4, 4, 32, 2.0, true),
    (QUIET_CHECK_BONUS, 24904, 2000, 30000, 1400.0, false),
    (LMR_COMPLEXITY_DIV, 159, 30, 500, 23.5, false),
    (CORR_HIST_DIV, 2312, 256, 4096, 192.0, true),
    (CORR_UPDATE_WEIGHT_MAX, 13, 4, 48, 2.2, true),
    // Was 32 (tp10→3). Now FIXED-POINT. Default 30 → eff 3.0 ≡ old behavior.
    (CORR_BONUS_CAP_DIV_10X, 11, 10, 160, 15.0, false),
    (CORR_HIST_GRAIN_T, 9, 1, 32, 1.55, false),
    // Floor lifted from 10 → 0 (audit 2026-05-19): SPSA converged 25, ~2%
    // from the floor. Lifting allows exploration of looser clamps.
    (CORR_HIST_ERR_MAX_10X, 19, 0, 640, 5.0, false),
    ...
    (ESCAPE_BONUS_R, 10292, 3000, 30000, 1350.0, false),
    ...
    // current defaults.
    (ESCAPE_BONUS_Q, 10449, 0, 30000, 1500.0, false),
    (ESCAPE_BONUS_MINOR, 12575, 0, 30000, 1000.0, false),
    // Null-move threat-escape bonus in quiet ordering (was hardcoded 8000).
    (NULL_THREAT_ESCAPE_BONUS, 13195, 0, 30000, 1000.0, false),
    ...
    (MOBILITY_DELTA_WEIGHT, 5, 0, 256, 8.0, false),
    (PROBCUT_KING_ZONE_MAX_10X, 38, 20, 90, 15.0, true),
    ...
    (DISCOVERED_ATTACK_BONUS, 4402, 0, 30000, 1500.0, false),
    ...
    (MVV_CAP_MULT, 27, 4, 64, 3.0, false),
    ...
    (CONT_HIST_MULT_10X, 2, 0, 80, 15.0, true),
    ...
    (PAWN_HIST_MULT_10X, 6, 0, 80, 10.0, false),
    (KNIGHT_FORK_BONUS, 10557, 0, 20000, 1000.0, false),
    ...
    (LMR_ENDGAME_PIECES_10X, 75, 45, 90, 15.0, true),
    ...
    (IIR_MIN_DEPTH_10X, 76, 5, 100, 15.0, true),          // was hardcoded 4; tune #743 converged to 2 (strong signal)
    ...
    (PROBCUT_MIN_DEPTH_10X, 44, 10, 120, 15.0, true),     // was hardcoded 5 (ProbCut activation gate)
    (SEE_CAP_DEPTH, 6, 3, 15, 1.5, true),         // was hardcoded 6 (SEE capture prune depth cap)
    (FUT_LMR_DEPTH, 5, 5, 20, 1.5, false),        // was hardcoded 10; tune #743 → 9
    (BAD_NOISY_DEPTH, 11, 4, 15, 1.5, true),       // was hardcoded 4 (BNFP depth cap)
    ...
    (NMP_MIN_DEPTH_10X, 74, 20, 200, 15.0, true),              // was hardcoded 3 (NMP activation gate, 2 sites)
    // Floor lifted from 10 → 0 (audit 2026-05-20): pinned at 25, 8% from floor.
    (HINDSIGHT_MIN_DEPTH_10X, 34, 0, 200, 15.0, true),        // was hardcoded 2 (hindsight reduction gate)
    (TT_CUTOFF_HALFMOVE_MAX, 100, 50, 100, 3.0, false),  // was hardcoded 90 (TT cutoff halfmove gate, 5 sites)
);
